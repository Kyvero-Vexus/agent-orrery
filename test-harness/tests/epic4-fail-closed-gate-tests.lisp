    ...
    (unwind-protect
         (progn
           ...
           (let* ((r (orrery/adapter:evaluate-epic4-fail-closed-gate dir "cd e2e && ./run-e2e.sh"))
             ...
             (is = 0 (length (orrery/adapter:e4fcr-reason-codes r)))
             (true (search "\"command_hash\":" json))
             (true (search "\"missing_scenarios\":[]" json))
             (true (search "\"reason_codes\":[]" json))))
    ...
    (unwind-protect
         (progn
           ...
           (let* ((r (orrery/adapter:evaluate-epic4-fail-closed-gate dir "cd e2e && ./run-e2e.sh"))
             ...
             (true (find "E4_MISSING_ARTIFACT_S1" (orrery/adapter:e4fcr-reason-codes r) :test #'string=))
             (true (search "\"missing_scenarios\":[" json))
             (true (search "\"reason_codes\":[" json))))
      (%cleanup-epic4-dir dir))))
    ...
    (unwind-protect
         (progn
           ...
           (let ((r (orrery/adapter:evaluate-epic4-fail-closed-gate dir "make e2e")))
             ...
             (true (find "E4_COMMAND_DRIFT" (orrery/adapter:e4fcr-reason-codes r) :test #'string=))
             (true (> (length (orrery/adapter:e4fcr-remediation r)) 0))))
      (%cleanup-epic4-dir dir))))
