    ...
    (unwind-protect
         (progn
           ...
           (let* ((v (orrery/adapter:evaluate-mcp-tui-replay-verdict dir "cd e2e-tui && ./run-tui-e2e-t1-t6.sh"))
                  (json (orrery/adapter:mcp-tui-replay-verdict->json v)))
             (true (orrery/adapter:mtrv-pass-p v))
             (true (> (orrery/adapter:mtrv-command-fingerprint v) 0))
             (true (search "\"rows\"" json))
             (true (search "\"command_fingerprint\"" json))
             (true (search "\"transcript_digest\":\"" json))
             (true (not (search "\"transcript_digest\":\"\"" json)))))
